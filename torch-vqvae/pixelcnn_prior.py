...
def generate_samples_pixelcnn_prior(args):

    writer = SummaryWriter('/proj/vondrick3/ishaan/pytorch-vqvae/logs/sampler')
    ...
    train_dataset = MiniImagenet('/proj/vondrick3/ishaan/old/miniimagenet', train=True,
                                 download=False, transform=transform)
    num_channels = 3

    model = VectorQuantizedVAE(num_channels, args.hidden_size_vae, args.k).to(args.device)
    print(os.getcwd())
    root_path = '/proj/vondrick3/ishaan/pytorch-vqvae'
    with open(os.path.join(root_path, 'models/models/vqvae_imagenet/best.pt'), 'rb') as f:
        state_dict = torch.load(f)
        model.load_state_dict(state_dict)

    if hasattr(train_dataset, '_label_encoder'):
        number_classes = len(train_dataset._label_encoder)

    else:
        number_classes  = len(train_dataset.classes)

    prior = GatedPixelCNN(args.k, args.hidden_size_prior,
        args.num_layers, n_classes=number_classes).to(args.device)

    if torch.cuda.device_count() > 1:
        prior = torch.nn.DataParallel(prior)
        prior.to('cuda')

    with open(os.path.join(root_path, 'models/models/vqvae_imagenet_prior_fast/prior.pt'), 'rb') as g:
        state_dict_prior = torch.load(g)
        prior.load_state_dict(state_dict_prior)


    label = torch.arange(10).expand(10, 10).contiguous().view(-1)
    label = label.long().cuda()

    latents = prior.module.generate(label, shape=IMAGE_SHAPE, batch_size=100)
    reconstructions = model.decode(latents).detach()
    reconstructions = reconstructions[:16]
    grid = make_grid(reconstructions.cpu(), nrow=8, range=(-1, 1), normalize=True)

    writer.add_image(f'reconstruction_trial', grid, 0)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str,
        help='filename containing the model', default='issue')
    ...
    #generate_samples_pixelcnn_prior(args)